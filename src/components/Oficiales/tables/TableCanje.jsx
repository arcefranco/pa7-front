...
useEffect(() => {
  ...
  function endEdit () {
      dispatch(endUpdate({Codigo: actualInEdit.current, categoria: 'Plan Canje'}))
  }
...
const nPages = Math.ceil(oficialesFiltered?.length / recordsPerPage)
    ...
    <table>
                ...
                {
                    currentRecords && currentRecords.map(e => <OficialItem key={e.Codigo} Categoria={'Plan Canje'} Codigo={e.Codigo} Nombre={e.Nombre} Activo={e.Activo} Inactivo={e.Inactivo}/>)
                }
            </table>
        <Pagination
            nPages = { nPages }
