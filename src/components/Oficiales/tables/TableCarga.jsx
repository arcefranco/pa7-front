...
const TableCarga = () => {
  ...
  useEffect(() => {
    dispatch(getOficialSelected({oficialName: 'Carga'}))
  }, [])
...
useEffect(() => {
  ...
  function endEdit () {
      dispatch(endUpdate({Codigo: actualInEdit.current, categoria: 'Carga'}))
  }
...
const nPages = Math.ceil(oficialesFiltered?.length / recordsPerPage)
    ...
    <table>
                ...
                {
                    currentRecords && currentRecords.map(e => <OficialItem key={e.Codigo} Categoria={'Carga'} Codigo={e.Codigo} Nombre={e.Nombre} Activo={e.Activo} Inactivo={e.Inactivo}/>)
                }
            </table>
        <Pagination
            nPages = { nPages }
