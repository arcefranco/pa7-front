    ...
    return (
        <div className={Title === 'Gerente' ? styles.item  : styles.subItem} style={{marginLeft: Title === 'Vendedor' && '15px'}}>
            ...
            <span className={styles.text} style={
                {color: Title === 'Gerente' ? 'cornflowerblue' : 
                Title === 'Supervisor' ? '#ed6464' : 
                Title === 'Team Leader' ? '#ed9064' : 
                'black'}} 
            onClick={() => setActivo(!activo)}>{Title}:</span><span> {Nombre}</span>
            {
            ...
            }
            {
                activo && Childrens ? Childrens.map(e => {
                    return (
                        <EstructuraItem Nombre={e.Nombre} Childrens={e.Childrens} Title={e.Title} Activo={e.Activo}/>
                    )
                }) : null
            }
        </div>
