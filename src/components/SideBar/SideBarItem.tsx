...
import { setToggle, logout } from "../../reducers/Login/loginSlice";
...
const SideBarItem = ({ item, index }) => {
  ...
  return (
    <div
      key={index}
      className={open ? styles.textOpen : styles.text}
      ...
      (user?.roles?.find((e) => e.rl_codigo === "1") && !item.unique) ? (
        <div className={styles[item.cName]} onMouseOver={showSideBar}>
          {item.path && item.cName !== "nav-text2" ? (
            <Link
              to={item.path}
              onClick={() => window.localStorage.removeItem("pageIndex")}
            >
            ...
            <span key={index} onClick={() => setOpen(!open)}>
              {toggle ? (
                <div
                  style={{
                    fontSize: "1.2em",
                    color: open ? "#3483fa" : "#545e65",
                    marginLeft: open ? "-.15em" : "0em",
                  }}
                  onMouseOver={showSideBar}
                >
                  {item.icon}
                ...
                >
                  <div
                    ...
                    style={{
                      color: open ? "#3483fa" : "#545e65",
                      alignItems:
                        "center" /*marginRight: open? ".5em" : ".2em"*/,
                    }}
                  >
                    {item.title}
                    {item.cName !== "nav-text2" ? (
                      <div
                        style={{ textAlign: "right" /* marginTop:"-1.5em" */ }}
                      >
                        {open ? (
