...
import {  endUpdate, updateTeamLeaders, deleteTeamLeaders, beginUpdate } from "../../../reducers/ConfigDatosGenerales/TeamLeaders/teamLeadersSlice";
...
const TeamLeadersItem = ({Codigo, Nombre, Supervisor, Activo}) => {
    ...
    const { statusNuevoTeamLeader, supervisores} = useSelector(state => state.teamLeaders)
    ...
    useEffect(() => {
        if(statusNuevoTeamLeader?.status === false) { //esta mirando el estado de sucursalStatus (inUpdate) para inhabilitar la edicion mientras este en false
            setEdit(false)
        }
    ...
    return (
        <tr>
            ...
            
            {
                ...
                <select style={{
                    border: 'none',
                    background: 'none'
            }} name="Supervisor" value={item.Supervisor} onChange={handleChange} id="">
                {
        ...
        <td>{
                ...
                : 
                item.Activo === 1 ? <input type="checkbox" checked /> : <input type="checkbox" />
            ...
            <td>
                {
                    item.Nombre !== Nombre || item.Activo !== Activo || item.Supervisor !== Supervisor ? 
