...
import { beginUpdate, endUpdate, deleteSupervisores, updateSupervisores } from "../../../reducers/ConfigDatosGenerales/Supervisores/supervisoresSlice";
import styles from '../../../styles/Table.module.css'
import Swal from "sweetalert2";
import { useEffect } from "react";
const SupervisoresItem = ({ Codigo, Nombre, Email, Gerente, EsMicro, VPM, Activo, Zona }) => {
    ...
    const { sucursalStatus, gerentes, zonas} = useSelector(state => state.supervisores)
    ...
    const handleChange = (e) => {
        ...
        if(name === "EsMicro" || name === "VPM" || name === "Activo" || name === "Gerente" || name === "Zona") {
    ...
    return (
        <tr>
            ...
            <td>{
                ...
                <input type="checkbox" name="EsMicro" value={item.EsMicro}  onChange={handleCheckChange}/> 
                : 
                item.EsMicro === 1 ? <input type="checkbox" readOnly={true} checked /> : <input type="checkbox" disabled readOnly />
            ...
            <td>{
                ...
                <input type="text" name="VPM" value={item.VPM}  onChange={handleChange}/> 
                : 
                <span>{VPM}</span>
            ...
            <td>{
                ...
                <select className={styles.select}  name="Zona" value={item.Zona} onChange={handleChange} id="">
                {
                    thisZona && <option value={thisZona.Codigo}>{thisZona.Nombre}</option> 
                }
                <option value="*">---</option>
                {
            ...
            <td>
                {
                    item.Nombre !== Nombre || item.Email !== Email || item.Gerente !== Gerente || item.EsMicro !== EsMicro ||
                    item.VPM !== VPM || item.Activo !== Activo || item.Zona !== Zona
                    ? 
