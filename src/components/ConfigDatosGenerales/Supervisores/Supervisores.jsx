...
import { getSupervisores, postSupervisores, resetStatus, endUpdate, getAllGerentes, getAllZonas } from "../../../reducers/ConfigDatosGenerales/Supervisores/supervisoresSlice";
...
const Supervisores = () => {
    ...
    const {supervisores, statusNuevoSupervisor, gerentes, zonas} = useSelector(state => state.supervisores)
    ...
    useEffect(() => {
        ...
        Promise.all([dispatch(getSupervisores()), dispatch(getAllGerentes()), dispatch(getAllZonas())])
...
useEffect(() => {
     ...
     useEffect(() => { //Manejar actualizaciones de vendedores (ABM) y su inUpdate
        ...
        function resetModal () {
         ...
         if(Object.keys(statusNuevoSupervisor).length){ 
     ...
     useEffect(() => {
        if(Object.keys(statusNuevoSupervisor).includes('codigo')) {
    ...
    const handleChange = (e) => {
        ...
        if(name === "Zona" || name === "VPM" || name === "Supervisor" || name === "Gerente") {
            parseValue = !isNaN(value) ? parseInt(value) : value
        }
        const newForm = {...newSupervisor,
    ...
    const currentRecords = supervisoresFiltered.slice(indexOfFirstRecord, 
        indexOfLastRecord);
    ...
    return (
        ...
        <div className={styles.container}>
            ...
            {
                modal && Object.keys(statusNuevoSupervisor).length && Object.keys(statusNuevoSupervisor).includes('status') ? 
                <ModalStatus status={statusNuevoSupervisor?.status} message={statusNuevoSupervisor?.message}/> :
                null
            } 
        ...
        <TableContainer>
            <table>
...

                {
                    ...
                    <tr>
                        <td></td>
                        <td><input type="text" name="Nombre" value={newSupervisor.Nombre} onChange={handleChange}/></td>
                        <td><input type="text" name="Email" value={newSupervisor.Email} onChange={handleChange}/></td>
                        <td>
                        ...
                        <td><input type="checkbox" name="EsMicro" value={newSupervisor.EsMicro}  onChange={handleCheckChange}/></td>
                        <td><input type="text" name="VPM" value={newSupervisor.VPM} onChange={handleChange}/></td>
                        <td><input type="checkbox" name="Activo" value={newSupervisor.Activo}  onChange={handleCheckChange}/></td>
                        <td>
                        <select name="Zona" className={styles.select} onChange={handleChange} id="">
                                <option value="*">---</option>
                                {
                                    zonas && zonas.map(e => <option value={e.codigo}>{e.Nombre}</option>)
                                }
                        </select>
                        </td>
...

                {
                    ...
                    Nombre={e.Nombre}
                    Email={e.Email}
                    EsMicro={e.EsMiniEmprendedor}
                    VPM={e.ValorPromedioMovil}
                    Gerente={e.Gerente}
                    Zona={e.Zona}
                    Activo={e.Activo}
                    />
