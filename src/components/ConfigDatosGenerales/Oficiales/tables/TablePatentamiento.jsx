...
useEffect(() => {
  ...
  function endEdit () {
      dispatch(endUpdate({Codigo: actualInEdit.current, categoria: 'Patentamiento'}))
  }
...
const nPages = Math.ceil(oficialesFiltered?.length / recordsPerPage)
    ...
    <table>
                ...
                {
                    currentRecords && currentRecords.map(e => <OficialItem key={e.Codigo} Categoria={'Patentamiento'} Codigo={e.Codigo} Nombre={e.Nombre} Activo={e.Activo} Inactivo={e.Inactivo}/>)
                }
            </table>
        <Pagination
            nPages = { nPages }
...
export default TablePatentamiento