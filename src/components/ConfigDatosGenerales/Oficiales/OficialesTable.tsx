...
const OficialesTable = () => {
  ...
  const { oficialesSelected, oficialStatus, oficialCategoria } = useSelector(
  ...
  useEffect(() => {
    ...
    if (oficialStatus && Object.keys(oficialStatus).length) {
  ...
  return (
    <div style={{ height: "100vh" }}>
      {modal &&
      oficialStatus &&
      Object.keys(oficialStatus).length &&
      Object.keys(oficialStatus).includes("status") ? (
        <ModalStatus
          status={oficialStatus?.status}
          message={oficialStatus?.message}
        />
      ) : null}
      <TitleLogo>
        <div>
