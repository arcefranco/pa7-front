...
const OficialItem = ({
  ...
  const thisUsuario = usuarios.find((e) => e.Usuario === IdUsuarioLogin);
  ...
  return (
    <tr>
        ...
        (Categoria === "Mora" && (
          <td>
            {edit ? (
              ...
              >
                {thisUsuario && (
                  <option value={thisUsuario.Usuario}>
                    {thisUsuario.Usuario}
                  </option>
                )}
                {usuarios &&
          ...
          <td>
            {edit ? (
              <select value={item.login} name="login" onChange={handleChange}>
                {thisLogin && (
                  <option value={thisLogin.Usuario}>{thisLogin.Usuario}</option>
                )}
                {usuarios &&
                  usuarios.map((e) => (
                    <option value={e.Usuario}>{e.Usuario}</option>
      ...
      <td>
        {typeof Activo === "number" ? ( //Pregunta si existe la propiedad, no si es un numero
          ...
          <input
            ...
            name="Inactivo"
            value={item.Inactivo}
            onChange={handleCheckInactivoChange}
          />
        ) : item.Inactivo === 0 ? (
          <input type="checkbox" checked readOnly={true} />
        ) : (
      ...
      <td>
        {item.Nombre !== Nombre ||
        item.Activo !== Activo ||
        item.Inactivo !== Inactivo ||
        item.Objetivo !== Objetivo ||
        item.Usuario !== IdUsuarioLogin ||
        item.TipoOficialMora !== TipoOficialMora ||
        item.HNMayor40 !== HNMayor40 ||
