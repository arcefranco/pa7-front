...
const VendedorItem = ({
  ...
  const { oficialesScoring, teamleader, oficialesMora, statusNuevoVendedor } =
  ...
  useEffect(() => {
    if (
      statusNuevoVendedor &&
      Object.keys(statusNuevoVendedor)?.includes("status")
    ) {
    ...
    <tr>
      ...
      <td>
        {!edit && FechaBaja && FechaBaja !== "00/00/0000" ? (
          ...
          />
        ) : !edit && (FechaBaja === "00/00/0000" || !FechaBaja) ? (
          <span></span>
        ) : (
          <input
            type="date"
      ...
      <td>
        {item.Activo === Activo &&
        item.Nombre === Nombre &&
        item.OficialS === OficialS &&
        item.OficialM === OficialM &&
        item.Escala === Escala &&
